/* $NCDId: @(#)nameserver.c,v 1.3 1994/07/05 18:59:37 greg Exp $ */
...
fprintf(stderr, "NETPATH sent to daemon is %s\n", nettype);
		...
		for(i=0; i< naddrs; i++)
		{
			...
			fd = OpenAndBind(NULL, -1, 0, nettype, Au_TLI_STREAM);
			if(fd  < 0)
			{
	...
	for(i=0; i < MAX_TRIES;i++)
	{
		...
		if((fd = OpenAndBind(NULL, -1, 0, nettype, Au_TLI_STREAM)) < 0)
		{
				PRMSG("Openandbind failed\n",0,0);
				break;	
		}
		...
		/*
		 * override whatever the X name server says is the port number
		 * with what we asked for for the audio server.
		 */
		{
		struct sockaddr_in *x = (struct sockaddr_in *)call.addr.buf;

		if(x->sin_family == AF_INET) {
			x->sin_port = htons(idisplay);
			if (tcpfamilyp != NULL) {
				*tcpfamilyp = AuNetworkInternet;
				*tcpremoteaddrlenp = 4;
				*tcpremoteaddrp = Aumalloc(*tcpremoteaddrlenp);
				/* This is a kludge.  What is the right way 
			...
			/* XXX anybody willing can fix this... */
			fprintf(stderr, 
			"_AusCallTliServer: non-TCP/IP not supported (yet)\n");
		}
		}
...
#define AuNETDB "lib/Xconnections"
                              ...
                              )
{
	...
	char *home;
	char	sysname[128] ;
	char	*procname = "Aulib/_XMakeStreamsConnection";
	...
	if(
	    ...
	    fd = ((*_AusStream[Au_LOCAL_STREAM].CallTheListener)
		  ("unix", *iserverp, "local"));
	    if (fd >= 0) {
		*familyp = FamilyLocal;
...
"sysname <%s>, nodename <%s>, release <%s>, version <%s>, machine <%s>\n",
		...
		if((netpath = (char *) getenv("NETPATH")) == NULL)
				netpath = "";
		return (*_AusStream[Au_TLI_STREAM].CallTheListener)
				(name, *iserverp, netpath);
	}
	...
	while((line = fgets(buf, 160, file)) != NULL)
	{
		...
		if(
			...
			nfound++;
		        fd = (*_AusStream[Au_TLI_STREAM].CallTheListener)
						(nodname, *iserverp, netype);
			if(fd >= 0)
			{
				fclose(file);
				return(fd);
